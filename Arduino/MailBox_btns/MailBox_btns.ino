...
byte sleepType = interruptableSleep;  //Set the Sleep Type
...
void loop() //Always Transmits a message then sleeps
{
  ...
  } else { //Interrupted Sleep
    //getData();
    if(digitalRead(mailIn)) { //Serial.println("You have Mail");
      msgToSend.sensor = 3;
      msgToSend.data = 1;
      transmit();
    }
    if(digitalRead(mailOut)) { //Serial.println("Mail Collected");
      msgToSend.sensor = 3;
      msgToSend.data = 0;
      transmit();
    }
